...
pub struct ContinuityProof {
    pub root_pub: [u8; 32],
    /// Ordered chain of rotation events (root → ... → current ops key).
    pub rotation_chain: Vec<[u8; 64]>,
    pub current_ops_pub: [u8; 32],
}

impl ContinuityProof {
    /// Verify the entire rotation chain from root to current operational key.
