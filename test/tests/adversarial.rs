...
fn transcript_v1_route_mutation_changes_hash() {
    ...
    let mutated = FlashTranscript {
        route_id: RouteId(mutated_route),
        ..base_v1()
    };
    assert_ne!(
        base.hash(),
...
fn transcript_v1_nonce_mutation_changes_hash() {
    let base = base_v1();
    let mutated = FlashTranscript {
        nonce: FreshnessNonce(base_v1().nonce.0 ^ 1),
        ..base_v1()
    };
    assert_ne!(
        base.hash(),
...
fn transcript_v2_sender_pub_mutation_changes_hash() {
    ...
    mutated_pub[0] ^= 0x01;
    let mutated = FlashTranscriptV2 {
        sender_ephemeral_pub: mutated_pub,
        ..base_v2()
    };
    assert_ne!(
        base.hash(),
...
fn transcript_v2_null_sender_pub_still_hashes() {
    let base = base_v2();
    let null_sender = FlashTranscriptV2 {
...
fn domain_transport_vs_relay_diverge() {
    ...
    assert_ne!(
        t, r,
        "Transport and Relay domains must produce distinct key output"
    );
}
...
fn domain_transcript_vs_transport_diverge() {
    ...
    assert_ne!(
        tr, tp,
        "Transcript and Transport domains must produce distinct output"
    );
}
...
async fn session_key_bytes_zeroized_after_dissolve() {
    ...
    let state = FlashSession::retrieve_state(&StubStateProvider, &[0x42u8; 32])
        .await
        .unwrap();
...
async fn two_sessions_have_independent_keys() {
    ...
    let s1 = FlashSession::open_and_send(
        FlashSession::retrieve_state(&StubStateProvider, &[0x01u8; 32])
            .await
            .unwrap(),
        b"session-one",
        &cache,
    ...
    .await
    .expect("session 1 must open");

    let s2 = FlashSession::open_and_send(
        FlashSession::retrieve_state(&StubStateProvider, &[0x02u8; 32])
            .await
            .unwrap(),
        b"session-two",
        &cache,
...
async fn route_id_is_not_derivable_from_ops_pub() {
    ...
    let session = FlashSession::open_and_send(
        FlashSession::retrieve_state(&StubStateProvider, &ops_pub)
            .await
            .unwrap(),
        b"route-test",
        &cache,
...
fn replay_window_boundary_minus_63_accepts() {
    ...
    assert!(
        w.check_and_insert(100 - 63), // 37 — last valid slot
...
async fn noise_relay_different_static_key_each_spawn() {
    ...
    for _ in 0..5 {
        let (_addr, pub_key) = spawn_noise_relay_listener()
            .await
...
fn ops_key_and_handshake_key_are_different_algorithms() {
    ...
    assert!(
        PublicKey(ops_kp.public).verify(b"test message", &sig),
        "Ed25519 ops key must sign and verify"
    );
    assert!(
        !PublicKey(eph_pub).verify(b"test message", &sig),
...
fn transcript_v2_format_byte_is_0x02() {
    ...
    assert_ne!(v2.hash(), [0u8; 32], "V2 transcript hash must be non-zero");
}
...
async fn flash_with_expired_handshake_ephemeral_rejected() {
    ...
    .await;
    assert!(
        matches!(result, Err(TransportError::HandshakeKeyExpired)),
...
fn ledger_duplicate_rotation_rejected() {
    ...
    ledger
        .register_identity(&record, &reg_sig)
    ...
    ledger
        .rotate_key(&genesis.k_ops_pub, &ops2.public, rot1.nonce, &rot1_sig)
...
fn handshake_ephemeral_with_past_published_at_accepted() {
    use scp_cryptography::keys::KeyPair;
    use scp_cryptography::x25519_generate_keypair;
    use scp_ledger_substrate::{handshake_sig_message, HandshakeEphemeral, SubstrateLedger};
    ...
    ledger
        ...
        .expect(
            "ephemeral with past published_at but future expires_at must be accepted — \
