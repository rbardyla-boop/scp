...
) -> ProviderPool<SubstrateLedger> {
    let mut pool =
        ProviderPool::new(SamplingStrategy::RandomK(k)).with_admissible_tracking(max_outstanding);
    for &b in providers {
...
fn rb1_accept_below_bound_refuse_at_bound() {
    ...
    for i in 0..3usize {
        ...
        assert!(
            result.is_ok(),
            "Sample {} below bound (outstanding={}) must succeed",
            i,
            i
        );
...
fn rb2_capacity_failure_before_raw_accounting() {
    ...
    assert_eq!(
        snap_after.selection_total, raw_selections_before,
        "raw selection_total must not change on ReceiptCapacityExhausted — \
         provider selection did not occur"
    );
    assert_eq!(
        snap_after.admissible_selection_total, adm_selections_before,
...
fn rb3_terminal_response_restores_capacity() {
    ...
    assert!(
        is_capacity_exhausted(&pool.sample_with_receipts(&mut rng)),
...
fn rb6_rotation_drain_restores_capacity_stale_receipts_inadmissible() {
    ...
    assert_eq!(
        pool.epoch_count(),
        1,
        "force_rotate must advance epoch to 1"
    );
...
fn rb11_multi_provider_quorum_partial_receipt_at_near_bound() {
    ...
    assert!(
        result.is_ok(),
        "3rd call with 1 outstanding < 3 bound must succeed"
    );
    ...
    assert_eq!(
        snap.admissible_selection_total, 5,
